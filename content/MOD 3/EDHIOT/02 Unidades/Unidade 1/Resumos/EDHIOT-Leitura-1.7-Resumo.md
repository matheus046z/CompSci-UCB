...
```mermaid
flowchart LR
    A["Automação de tarefas<br/>Regras e RPA"]
    B["Automação de processos<br/>Fluxos de trabalho"]
    C["Operações digitais<br/>do negócio"]
    D[Hiperautomatização]
...
flowchart TD
    A[Screen scraping]
    B[Automação de fluxos]
    C[Inteligência artificial]
    D[RPA]
    E[Hiperautomatização]
